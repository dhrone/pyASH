...
class GPIO(object):
    ...
    def output(pin, value):
        print ('Setting {0} to {1}'.format(pin, value))
...
class cloudLight(Device):
    ...
    def __init__(self, gpio=14):
        self._gpio = gpio
        super(cloudLight, self).__init__(name = 'Cloud Light', properties = { 'powerState': 'OFF' })
...
#    import RPi.GPIO as GPIO
    ...
    myCloudLight = cloudLight()
    eventQueue = queue.Queue()
    myCloudLight.start(eventQueue)

    myCloudLight.update('powerState', 'ON')
    time.sleep(2)
    myCloudLight.update('powerState', 'OFF')
